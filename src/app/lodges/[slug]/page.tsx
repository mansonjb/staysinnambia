...
export default async function LodgePage({
  ...
  const region = getRegion(lodge.regionSlug);
  const relatedLodges = getLodgesForRegion(lodge.regionSlug).filter((l) => l.slug !== lodge.slug);
  ...
  return (
    <>
      ...
      <nav className="mx-auto max-w-[1400px] px-6 pt-6 text-[13px] text-charcoal/50 sm:px-10">
        ...
        <Link href="/best-lodges-in-namibia" className="hover:text-rust">
          Lodges
        </Link>
        <span className="mx-2">/</span>
