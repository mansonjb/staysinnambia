...
import { getLodgeDe, getLodgesForRegionDe, getRegionDe, lodgesDe } from "@/lib/data.de";
import { SITE_URL } from "@/lib/site";

export function generateStaticParams() {
  return lodgesDe.map((l) => ({ slug: l.slug }));
}
...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: {
      canonical: `/de/lodges/${lodge.slug}`,
...
export default async function LodgePageDe({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  ...
  const region = getRegionDe(lodge.regionSlug);
  const relatedLodges = getLodgesForRegionDe(lodge.regionSlug).filter((l) => l.slug !== lodge.slug);
    ...
    <>
      <JsonLd
        data={breadcrumbJsonLd([
          ...
          { name: lodge.name, url: `${SITE_URL}/de/lodges/${lodge.slug}` },
        ])}
      />
      <JsonLd
        data={lodgingBusinessJsonLd({
          name: lodge.name,
          description: lodge.description,
          url: `${SITE_URL}/de/lodges/${lodge.slug}`,
          image: lodge.image,
      ...
      <nav className="mx-auto max-w-[1400px] px-6 pt-6 text-[13px] text-charcoal/50 sm:px-10">
        ...
        <Link href="/de/best-lodges-in-namibia" className="hover:text-rust">
          Lodges
        </Link>
        <span className="mx-2">/</span>
          ...
          <div>
            ...
            {region && (
              <div className="mt-8 flex flex-wrap items-center gap-4">
                ...
                >
                  Weitere Unterkünfte in {region.name}
                </Link>
                <a href="#availability">
        ...
        <section id="availability" className="mx-auto max-w-[1400px] px-6 py-16 sm:px-10">
          <Reveal>
            <h2 className="text-2xl font-bold text-charcoal">
              Karte der Unterkünfte in der Nähe von {lodge.name}
            </h2>
            <p className="mt-2 max-w-lg text-sm leading-relaxed text-charcoal/60">
              Echte, buchbare Unterkünfte rund um {region.name}, bereitgestellt von unserem
      ...
      {relatedLodges.length > 0 && (
        <section className="bg-sand/20 py-16 sm:py-20">
          <div className="mx-auto max-w-[1400px] px-6 sm:px-10">
            <Reveal>
              <h2 className="text-2xl font-bold text-charcoal">
                Weitere Lodges in {lodge.region}
              </h2>
            </Reveal>
