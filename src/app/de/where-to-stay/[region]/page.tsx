...
import { getLodgesForRegionDe, getRegionDe, regionsDe } from "@/lib/data.de";
import type { Region } from "@/lib/data";
import { SITE_URL } from "@/lib/site";

const RELATED_LINKS_DE = [
  { title: "Beste Lodges in Namibia", href: "/de/best-lodges-in-namibia" },
  { title: "Alle Reiserouten", href: "/de/itineraries" },
];

export function generateStaticParams() {
  return regionsDe.map((r) => ({ region: r.slug }));
}
...
export async function generateMetadata({
  ...
  const region = getRegionDe(slug);
  if (!region) return {};

  return {
    title: `Unterkünfte in ${region.name}`,
    description: region.description,
    alternates: {
      canonical: `/de/where-to-stay/${region.slug}`,
...
export default async function RegionPageDe({
  params,
}: {
  params: Promise<{ region: string }>;
}) {
  ...
  const regionLodges = getLodgesForRegionDe(region.slug);

  const nearbyMatches = region.nearby.map((entry) => ({
    text: entry,
    region: regionsDe.find(
  ...
  const otherRegions =
    pairedRegions.length > 0
      ? pairedRegions
      : regionsDe.filter((r) => r.slug !== region.slug).slice(0, 3);
    ...
    <>
      <JsonLd
        data={breadcrumbJsonLd([
          ...
          { name: region.name, url: `${SITE_URL}/de/where-to-stay/${region.slug}` },
        ])}
      />
      ...
      <nav className="mx-auto max-w-[1400px] px-6 pt-6 text-[13px] text-charcoal/50 sm:px-10">
        ...
        <Link href="/de/where-to-stay/namibia" className="hover:text-rust">
          Unterkünfte
        </Link>
        <span className="mx-2">/</span>
      ...
      <section className="mx-auto max-w-[1400px] px-6 py-12 sm:px-10">
        <Reveal className="grid grid-cols-1 gap-6 sm:grid-cols-2">
          <div className="rounded-2xl border border-black/5 bg-olive/5 p-6">
            <h2 className="text-lg font-bold text-olive">Warum hier übernachten</h2>
          ...
          <div className="rounded-2xl border border-black/5 bg-rust/5 p-6">
            <h2 className="text-lg font-bold text-rust">Wissenswert</h2>
            <ul className="mt-4 space-y-2.5">
              {region.cons.map((c) => (
        ...
        <Reveal className="mt-6 rounded-2xl border border-black/5 bg-white p-6 shadow-sm">
          <p className="text-[14px] text-charcoal/60">
            <span className="font-semibold text-charcoal">Eher meiden, wenn:</span> {region.avoidIf}
          </p>
        </Reveal>
      </section>
      ...
      <section className="mx-auto max-w-[1400px] px-6 py-12 sm:px-10">
        <Reveal>
          <h2 className="text-3xl font-bold tracking-tight text-charcoal sm:text-4xl">
            Wie viele Tage für {region.name}?
          </h2>
          <p className="mt-3 max-w-lg text-sm leading-relaxed text-charcoal/60">
            Wählen Sie die Länge, die zu Ihrer Reise passt: jede Option ist ein echter Kompromiss,
            keine Checkliste.
          </p>
        </Reveal>
        <div
        ...
        <section className="bg-sand/20 py-16 sm:py-20">
          <div className="mx-auto max-w-[1400px] px-6 sm:px-10">
            <Reveal>
              ...
              <p className="mt-3 max-w-lg text-sm leading-relaxed text-charcoal/60">
                Keine Preise. Keine gefälschten Bewertungen. Ausgewählt allein nach Lage, Service
                und Qualität.
              </p>
            </Reveal>
      ...
      <section className="bg-sand/20 py-16 sm:py-20">
        <div className="mx-auto max-w-[1400px] px-6 sm:px-10">
          <Reveal>
            <h2 className="text-3xl font-bold tracking-tight text-charcoal sm:text-4xl">
              Oft kombiniert mit {region.name}
            </h2>
          </Reveal>
          ...
          {landmarks.length > 0 && (
            <Reveal className="mt-8 flex flex-wrap items-center gap-2">
              <span className="text-[13px] font-semibold text-charcoal/50">
                Ebenfalls gut kombinierbar mit:
              </span>
              {landmarks.map((l) => (
                <span
                  key={l}
      ...
      <section className="mx-auto max-w-[1400px] px-6 pb-20 sm:px-10 sm:pb-28">
        <Reveal className="rounded-3xl bg-rust px-6 py-14 text-center sm:px-16 sm:py-16">
          <h2 className="mx-auto max-w-xl text-3xl font-bold tracking-tight text-ivory">
            Bereit, {region.name} zu entdecken?
          </h2>
          <p className="mx-auto mt-4 max-w-lg text-[15px] leading-relaxed text-ivory/80">
            Durchsuchen Sie jede Unterkunft auf der Karte oben, oder sehen Sie sich alle neun
            Regionen an.
          </p>
          <Link
